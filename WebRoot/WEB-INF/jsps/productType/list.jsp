...
<script type="text/javascript">
    ...
    function udpatePT(ptId){
        var diag = new Dialog();
        diag.Width = 850;
        diag.Height = 400;
        diag.ShowButtonRow=true;
        diag.Title = "修改商品类别";
        diag.URL = "${path}/productType_update?productType.productTypeId="+ptId;
        diag.OKEvent = function(){
            var win = diag.innerFrame.contentWindow;
            //调用提交表单的方法获得返回值
            var result = win.submitForm();
    ...
    function deletePT(ptId){
        Dialog.confirm('您在删除分类的同时，会把分类下商品一同删除，确认要删除吗?',function(){
            window.location.href = "${path}/productType_delete?productType.productTypeId="+ptId;
        });
    }
</script>
						...
						<tr align="center" bgcolor="#FFFFFF">
							...
							<td>
								...
								<span style="line-height:12px; text-align:center;"> 
									<a href="javascript:void(0)" class="xiu" onclick="udpatePT(<s:property value="#pt.productTypeId"/>)">修改</a>
								</span> 
								<img src="${path}/images/icon_04.gif" /> 
								<span style="line-height:12px; text-align:center;"> 
									<a href="javascript:void(0)" class="xiu" onclick="deletePT(<s:property value="#pt.productTypeId"/>)">删除</a>
								</span>
							</td>
						</tr>
					</s:iterator>
