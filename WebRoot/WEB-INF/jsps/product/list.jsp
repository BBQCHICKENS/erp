...
<script type="text/javascript">
	...
	function updateProduct(ptId) {
        var diag = new Dialog();
        diag.Width = 850;
        diag.Height = 400;
        diag.ShowButtonRow=true;
        diag.Title = "商品添加";
        diag.URL = "${path}/product_update?product.productId="+ptId;
        diag.OKEvent = function(){
            var win = diag.innerFrame.contentWindow;
            //调用提交表单的方法获得返回值
            var result = win.submitForm();
    ...
    function deleteProduct(ptId){
        Dialog.confirm('确认要删除吗?',function(){
            window.location.href = "${path}/product_delete?product.productId="+ptId;
        });
    }
</script>
