...
<script type="text/javascript">
	...
	function viewDetail(orderId){
		...
		diag.show();
        diag.addButton("next","审核意见",function(){
                 var diag1 = new Dialog();
                  diag1.Width = 850;
                  diag1.Height = 400;
                 diag1.ShowButtonRow=true;
                 diag1.Title = "审核意见";
                 diag1.URL = "${path}/consoleLog_log?query.entityId="+orderId+"&query.tableName="+"order_model"+"&query.optType="+"审核订单";
	...
	function auditOrderCheck(orderId){
		var diag = new Dialog();
		diag.Width = 450;
		diag.Height = 200;
		diag.ShowButtonRow=true;
		...
		diag.OKEvent = function(){
			...
			if(result=="success"){
                diag.close();
                location.href="${path }/orderModel_checklist?query.orderType=1&query.orderState=1";
			}
		};
		diag.show();
	...
	function auditOrder(orderId, orderState, note){
		var result = ""
		$.ajax({
			url:"${path}/ajax_orderModel_examine",
			type:"post",
			data:{
						...
						<tr align="center" bgcolor="#FFFFFF">
							...
							<td>
								...
								<s:if test="#order.orderState == 1">
									<input type="button" onclick="auditOrderCheck(<s:property value="#order.orderId"/>)"  value="审核">
								</s:if>
							</td>
							<!-- <td>未审核</td> -->
						</tr>
