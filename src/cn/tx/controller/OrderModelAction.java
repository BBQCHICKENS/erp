...
public class OrderModelAction extends BaseAction {
	...
	public void ajax_orderModel_submitOrder() throws IOException{
     ...
     public void ajax_orderModel_examine() throws  Exception{
	     ActionContext actionContext = ActionContext.getContext();
         Map<String, Object> session2 = actionContext.getSession();
         Emp emp = (Emp) session2.get("user");
         OrderModel order1 = this.orderModelService.getObj(this.order.getOrderId());
         order1.setOrderChecker(emp);
         order1.setCheckTime(new Date());
         order1.setOrderState(this.order.getOrderState());
